...
const { MAX_UPLOAD_FILES, MAX_UPLOAD_SIZE_BYTES, MAX_UPLOAD_SIZE_MB, UPLOAD_DIR } = require('../config');
const { beginTransaction, commitTransaction, getDb, rollbackTransaction, runDb, allDb } = require('../db');
const { assignmentSelectSql, formatAssignment, getEffectiveStatus, parseJson, validateSubmittedFieldValue } = require('../utils/assignmentUtils');
const {
  buildUploadFilename,
...
// MIME type whitelist for uploaded files
...
const uploadAssignmentFiles = upload.fields([
  ...
  { name: 'homeworkFile', maxCount: MAX_UPLOAD_FILES },
]);
...
function isNameIdentityField(field) {
  return (
    getNormalizedFieldText(field, 'key') === 'studentName' ||
...
function isStudentIdIdentityField(field) {
  return (
    getNormalizedFieldText(field, 'key') === 'studentId' ||
...
function isPhoneIdentityField(field) {
  return (
    getNormalizedFieldText(field, 'key') === 'phone' ||
...
function isEmailIdentityField(field) {
  return (
    getNormalizedFieldText(field, 'key') === 'email' ||
...
function isSubmissionIdentityField(field) {
  return (
    field?.enabled !== false &&
    field?.visible !== false &&
    (isNameIdentityField(field) || isStudentIdIdentityField(field) || isPhoneIdentityField(field) || isEmailIdentityField(field))
  );
}
    ...
    async (req, res) => {
      ...
      try {
        ...
        for (const file of uploadedFiles) {
          const ext = path
            .extname(file.decodedOriginalName || file.originalname)
            .replace(/^\./, '')
            .toLowerCase();
          if (allowed.size && !allowed.has(ext)) throw new Error('BAD_EXT');
          if (assignment.enableLimit && file.size > Math.min(Number(assignment.maxFileSizeMb || MAX_UPLOAD_SIZE_MB), MAX_UPLOAD_SIZE_MB) * 1024 * 1024)
